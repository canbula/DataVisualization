...
class MutabilityTest:
    ...
    def __call__(self, *args, **kwargs):
        ...
        for i in range(len(self._tests)):
            test_str = f"[TEST] {i+1:2d} / {len(self._tests):2d} {self._tests[i]['test_name']} :"
            if not isinstance(self._tests[i]["input"], self.__types):
                logging.error(f"{test_str} Type not supported")
                continue
            if not isinstance(self._tests[i]["expected"], bool):
                logging.error(f"{test_str} Expected value not supported")
                continue
            if not isinstance(self._tests[i]["test_name"], str):
                logging.error(f"{test_str} Test name not supported")
                continue
            try:
                assert is_mutable(self._tests[i]["input"]) is self._tests[i]["expected"]
...
def main():
    with MutabilityTest() as mutability_test:
        ...
        mutability_test._tests.append({"input": pd.DataFrame(data= {'col1': [1, 2], 'col2': [3, 4]}), "expected": True, "test_name": "Pandas DataFrame test"})
        mutability_test()
